...
export let loader: LoaderFunction = async ({ request }) => {
  const userList = await getUserList([1, 3]); // Lấy danh sách tất cả người dùng đã đăng ký
                          ...
                          <h6 className="mb-0">
                            <div className="font-semibold">
                              <span className="w-fit relative">
                                {user.name}
                                {user.groupPitches.find((gp: any) => {
                                  return (
                                    user.id == gp.ownerId && gp.status == 2
                                  );
                                }) ? (
                                  <div
                                    className={`absolute left-full text-nowrap -top-2 right-auto text-xs px-3 flex gap-1 items-center w-fit mb-2 py-[2px]  rounded-full text-green-800 bg-green-200`}
                                  >
                                    Chủ sân
                                  </div>
                                ) : (
